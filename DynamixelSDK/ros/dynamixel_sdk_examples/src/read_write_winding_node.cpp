...
void setVelocityCallback(const dynamixel_sdk_examples::SetVelocity::ConstPtr & msg)
{
  ...
  // ========================== TORQUE ENABLE ===========================
  dxl_comm_result = packetHandler->write1ByteTxRx(
    portHandler, (uint8_t)msg->id, ADDR_TORQUE_ENABLE, 1, &dxl_error);
  if (dxl_comm_result != COMM_SUCCESS) {
    ROS_ERROR("Failed to enable torque for Dynamixel ID %d", (uint8_t)msg->id);
  }
  // ====================================================================
  // ============================== LED ON ==============================
  dxl_comm_result = packetHandler->write1ByteTxRx(
    portHandler, (uint8_t)msg->id, ADDR_LED_ON_OFF, 1, &dxl_error);
  if (dxl_comm_result != COMM_SUCCESS) {
    ROS_ERROR("Failed to enable LED for Dynamixel ID %d", DXL1_ID);
  }
  // ====================================================================
...
int main(int argc, char ** argv)
{
  ...
  ros::init(argc, argv, "read_write_winding_node");
  ros::NodeHandle nh;
